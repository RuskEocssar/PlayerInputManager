...
## アイテムデータを反映
    ...
    # 紐づけされたアイテムエンティティを検索
        ...
        # 見つからない場合は新たに召喚
        execute if score #pliH. pliS. matches 0 run function zz.player_input:root/summon
    # 後処理
...
## クリックの判定
    ...
    # 長押しによるイベントがない場合
    execute if data storage player_input: data{hold:"",hold_init:"",hold_end:""} run function zz.player_input:item/right_click/event_wohold
    # ある場合
    execute unless data storage player_input: data{hold:"",hold_init:"",hold_end:""} run function zz.player_input:item/right_click/event_whold
    # クリック判定用時間のリセット
