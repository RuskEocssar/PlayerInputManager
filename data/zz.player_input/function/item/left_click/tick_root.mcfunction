#> zz.player_input:item/left_click/tick_root
# タップ判定用のスコア加算
# @within
...
## プレイヤーから実行
    ...
    execute on origin run function zz.player_input:item/left_click/tick_player
    
## ホールドが終了したら削除
    execute if score #pinH.player pinS. matches 1 run scoreboard players reset @s pinS.time.left_click