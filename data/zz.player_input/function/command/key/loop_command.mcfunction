...
## コマンド入力ごとに確認
    ...
    execute if score #pliH.check_command pliS. matches 1 store result score #pliH.check_command pliS. run function zz.player_input:command/key/check_time
    # 判定が成功した場合
        # まだコマンド判定が残っている場合
        execute if score #pliH.check_command pliS. matches 1 run data remove storage player_input:zz _com.list[0]
        execute if score #pliH.check_command pliS. matches 1 if data storage player_input:zz _com.list[-1] run function zz.player_input:command/key/success
        # 最後まで成功した場合
