...
## アイテムを使用可能にする
    # 判定が取れないアイテムの場合、設定する
    execute as 4fe002bb-0-6-0-a00000001 unless data storage player_input: item.components."minecraft:consumable" unless data storage player_input: item.components."minecraft:blocks_attacks" unless predicate zz.player_input:usable run function zz.player_input:slot/set_usable
