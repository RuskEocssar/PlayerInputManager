...
## イベント実行
    ...
    # ホールド
    execute if entity @s[tag=pliT.hold.left] if predicate zz.player_input:hold if predicate zz.player_input:click run function zz.player_input:key/event/hold_end
    execute if entity @s[tag=pliT.hold.left] if predicate zz.player_input:hold if predicate zz.player_input:click run tag @s remove pliT.hold.left

## スコア加算
    scoreboard players add @s pliS.time.left 1
    scoreboard players add @s[scores={time.left=0..}] time.left 1
    execute as @s[scores={time.left=0..},predicate=zz.player_input:click] run scoreboard players set @s time.left -1
