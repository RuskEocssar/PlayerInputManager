...
## アイテムデータを反映
    ...
    # 紐づけされたアイテムエンティティを検索
        ...
        # 見つからない場合は新たに召喚
        execute if score #pliH.check_player pliS. matches 0 as 4fe002bb-0-2-0-a00000001 if function zz.player_input:root/summon on passengers if function zz.player_input:root/check_origin run function zz.player_input:key/event/delete_one_root with storage player_input:zz in
    # 後処理
