...
## イベント実行
    ...
    # ホールド
    execute if entity @s[tag=pliT.hold.backward] if predicate zz.player_input:hold if predicate zz.player_input:click run function zz.player_input:key/event/hold_end
    execute if entity @s[tag=pliT.hold.backward] if predicate zz.player_input:hold if predicate zz.player_input:click run tag @s remove pliT.hold.backward

## スコア加算
    scoreboard players add @s pliS.time.backward 1
    scoreboard players add @s[scores={time.backward=0..}] time.backward 1
    execute as @s[scores={time.backward=0..},predicate=zz.player_input:click] run scoreboard players set @s time.backward -1
