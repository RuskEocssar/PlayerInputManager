#> zz.player_input:key/sneak/tick
# 常時実行
# @within
...
## プレイヤーから実行
    ...
    execute on origin run function zz.player_input:key/sneak/tick_player

## スコア設定
    scoreboard players add @s pliS. 1
    
## ホールドが終了したら削除
    execute if score #pliH.player pliS. matches 1 run scoreboard players reset @s pliS.time.sneak