...
プレイヤーの前後左右移動やジャンプ、スニーク、右や攻撃などの入力において、シングルタップ、ダブルタップ、長押しなどに応じたコマンドの実行を簡単に設定できるようにするためのデータパックです。
...
## 主な機能
- 前後左右移動(WASD)、ジャンプ(space)、スニーク(shift)、ダッシュ(ctrl)、アイテムの右の操作に対応
...
# 使い方
プレイヤーのキー入力にイベントを設定する場合と、アイテムの右にイベントを設定する場合で方法が異なります。
