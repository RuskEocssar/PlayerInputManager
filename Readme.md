...
- [使い方](#使い方)
  ...
  - [アイテム左クリックのイベント設定](#アイテム左クリックのイベント設定)
  - [共通設定](#共通設定)
  - [ファンクション](#ファンクション)
- [スコアボード](#スコアボード)
- [使用例](#使用例)
...
"minecraft:custom_data" : {
    left_click : {
        ...
        hide : <int>,               | (3) ツールチップの秘匿レベルの設定。カスタムエンチャントを隠す場合、必然てきにほかのエンチャントも隠れてしまうため。1:隠さない、2:ツールチップは非表示、エンチャントの煌めきは表示、3:ツールチップとエンチャントの煌めきともに非表示
    }
}
```
...
| スコアボード      | 名前               | 説明                                                                                                                                             |
| ----------------- | ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| `time.right_click` | 右クリックの保持時間 | 右クリックにおいて、ホールドしている場合は最初のタップから何tick経過したか、ダブルタップの場合は一回目のタップから何tick後かを意味する。 |
| `time.left_click` | 左クリックの保持時間 | 左クリックにおいて、ホールドしている場合は最初のタップから何tick経過したか、ダブルタップの場合は一回目のタップから何tick後かを意味する。 |
...
| `mode.right_click` | 右クリックの保持時間 | 右クリックにおいて、最後に行ったアクションの状態を記録する。0:シングルタップ、 1:ダブルタップ、 2:ホールド開始、 3:ホールド終了            |
| `mode.left_click` | 左クリックの保持時間 | 左クリックにおいて、最後に行ったアクションの状態を記録する。0:シングルタップ、 1:ダブルタップ、 2:ホールド開始、 3:ホールド終了            |
